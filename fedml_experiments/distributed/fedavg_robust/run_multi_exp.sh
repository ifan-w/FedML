...
ATTACKER_EPOCH=160
ATTACKER_LR=0.1
echo ----- attacker lr $ATTACKER_LR, attackr ep $ATTACKER_EPOCH -----
mpirun -np $PROCESS_NUM -hostfile ./mpi_host_file python3 ./main_fedavg_robust.py \
  ...
  --title "SA-$ROUND-$ATTACKER_LR-$ATTACKER_EPOCH-$ATTACKER_OPTIM-1"
...
mpirun -np $PROCESS_NUM -hostfile ./mpi_host_file python3 ./main_fedavg_robust.py \
  ...
  --client_optimizer $OPTIM \
  --note "Single shot attack, attacker lr-$ATTACKER_LR ep-$ATTACKER_EPOCH" \
  --title "SA-$ROUND-$ATTACKER_LR-$ATTACKER_EPOCH-$ATTACKER_OPTIM-2"
