...
def partition_data(dataset, datadir, partition, n_nets, alpha, logger=None):
    if logger == None:
        logger = logging
...
# def get_dataloader(dataset, datadir, train_bs, test_bs, dataidxs=None, parent=None):
#     return get_dataloader_CIFAR10(datadir, train_bs, test_bs, dataidxs, parent=None)
...
def get_dataloader_CIFAR10(datadir, train_bs, test_bs, dataidxs=None, parent=None):
    dl_obj = CIFAR10_truncated
...
def load_partition_data_cifar10(dataset, data_dir, partition_method, partition_alpha, client_number, batch_size, logger=None):
    if logger == None:
        logger = logging
    X_train, y_train, X_test, y_test, net_dataidx_map, traindata_cls_counts = partition_data(dataset,
                                                                                             data_dir,
    ...
    # train_data_global, test_data_global = get_dataloader(dataset, data_dir, batch_size, batch_size)
    train_ds_global, test_ds_global = get_dataset(data_dir)
    train_data_global, test_data_global = get_dataloader(train_ds_global, test_ds_global, batch_size, batch_size)
